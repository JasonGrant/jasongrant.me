...
import styles from "./OptionsExploredDeck.module.css";

// Deck-only copy of work/app-shell/OptionsExplored, enlarged for the stage
// and with a segmented control so one option renders at a time instead of
// all three stacked. Kept as a full copy — not a wrapped import — so its
// type scale can diverge freely from the site's without touching the
// public case study.
...
const TABS: { id: OptionId; label: string }[] = [
  { id: "accordion", label: "Scrolling accordion" },
  { id: "buttons", label: "Vertical nav buttons" },
  { id: "dropdown", label: "Dropdown" },
];

export function OptionsExploredDeck() {
  const [active, setActive] = useState<OptionId>("accordion");
  const option = OPTIONS.find((o) => o.id === active) ?? OPTIONS[0];

  return (
    <div className={styles.wrap}>
      <div className={styles.segmented} role="radiogroup" aria-label="Option explored">
        {TABS.map((t) => (
          <button
            key={t.id}
            type="button"
            ...
            className={classNames(styles.segment, active === t.id && styles.segmentOn)}
              ...
              {CRITERIA.map((c) => {
                ...
                return (
                  <li key={c.id} className={styles.criteriaRow}>
                    <span
                      className={classNames(styles.pill, pass ? styles.pillDo : styles.pillDont)}
                    >
