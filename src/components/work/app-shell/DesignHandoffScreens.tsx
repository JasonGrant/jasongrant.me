...
const SLIDES: StepperSlide[] = [
  {
    ...
    caption:
      "The pane's landmark structure, annotated down to the ARIA: aria-labelledby ties the region to its visible heading, aria-live announces tab switches without interrupting, and the hidden attribute, not CSS alone, removes inactive panels from both the visual and accessibility tree.",
    width: 2800,
    height: 1575,
  },
  {
    ...
    caption:
      "Every measurement in the pane traces back to a token (spacing, sizing, borders) so engineering builds against the same system the design lives in, not a one-off set of pixel values.",
    width: 2800,
    height: 1575,
  },
  {
    ...
    caption:
      "The old nav and the new one, side by side with matching numbered callouts: every element in the before has a named counterpart in the after, so nothing in the redesign is unaccounted for.",
    width: 2800,
    height: 1575,
  },
  {
    ...
    caption:
      "The sidebar decomposed into its actual containers: collapsed and expanded widths, the three vertical zones, and the class name each one maps to in code.",
    width: 2800,
    height: 1575,
  },
  {
    ...
    title: "Top zone",
    caption:
      "The top zone's pieces (logo, create button, nav item, nav group) each specified across every state they can be in, with the markup and ARIA that state depends on.",
    width: 2800,
    height: 1575,
  },
  {
    ...
    title: "Center zone",
    caption:
      "The collapse trigger's full state machine (six states in total) plus the button markup underneath: a real button, not a styled div, with aria-expanded doing the work.",
    width: 2800,
    height: 1575,
  },
  {
    ...
    title: "Bottom zone",
    caption:
      "The bottom zone's secondary actions (sound, help, profile) get the same state-by-state treatment as primary navigation, so nothing at the bottom of the rail is an afterthought.",
    width: 2800,
    height: 1575,
  },
];
